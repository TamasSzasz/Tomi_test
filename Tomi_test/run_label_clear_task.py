...
try:
    ...
    cv2.displayOverlay("Test", "Test QT", 100)
except:
    WITH_QT = False
...
parser.add_argument('--sort', default=True, action='store_true', help="If true, shows images in order.")
...
parser.add_argument('--zoom_ratio', default=2, type=float, help="Zoom ratio")
parser.add_argument("--root_dir", default="")
parser.add_argument("--make_copy", default=0)
...
if "\images" in args.root_dir:
    img_dir = args.root_dir
    bb_dir = args.root_dir.replace("images", "labels")
else:
    if args.root_dir != "":
        img_dir = os.path.join(args.root_dir, img_dir)
        bb_dir = os.path.join(args.root_dir, bb_dir)
...
unzoom = False
show_boxes = True
show_only_one_per_second = False
number_of_tick = 0
...
def draw_bboxes_from_file(tmp_img, txt_path, width, height):
    global img_objects
    global number_of_tick
    global show_boxes
    global show_only_one_per_second
    img_objects = []
    if os.path.isfile(txt_path) and show_boxes:
        with open(txt_path) as f:
            content = f.readlines()
        if show_only_one_per_second:
            number_of_tick += 2
            if number_of_tick > (len(content) * 10):
                number_of_tick = 0
        ...
        for line in content:
            ...
            if show_only_one_per_second:
                ...
                if cnt < number_of_tick or cnt > number_of_tick + 9:
                    continue
...
def zoom_resize(tmp_img, height, width, zoom_ratio):
    tmp_img = cv2.resize(tmp_img, (int(height * zoom_ratio), int(width * zoom_ratio)))
...
def clear_all_bb():
    ...
    MsgBox = messagebox.askyesno(
        ...
        message='This will clear all bounding boxes permanently. \nAre You sure?',
        icon='warning')
    if MsgBox == True:
...
image_list.extend(glob.glob(img_dir + '*.png'))
#print(image_list)
image_list.sort()
# if args.make_copy == 1:
#     for i in image_list:
#         for r in ["jpg", "jpeg", "png"]:
#             t = i.replace("images", "labels").replace(r, "txt")
#             if os.path.exists(t):
#                 mydir = os.path.dirname(i)
#                 new_images_dir = mydir.replace("images\\", "copy\\images\\")
#                 if not os.path.exists(new_images_dir):
#                     os.mkdir(new_images_dir)
#                 new_labels_dir = new_images_dir.replace("images", "labels")
#                 if not os.path.exists(new_images_dir, new_labels_dir):
#                     os.mkdir(new_labels_dir)
#                 new_image_path = os.path.join(new_images_dir, os.path.basename(i))
#                 shutil.copyfile(i, new_image_path)
#                 new_label_path = new_image_path   ... fix
#                 shutil.copyfile(t, new_image_path)
if not args.sort:
...
class_rgb = [
    ...
    (255, 0, 255), (192, 192, 64), (64, 64, 255), (128, 0, 0),
    (128, 128, 0), (0, 255, 0), (128, 0, 128), (0, 0, 128), (0, 128, 128),
    (128, 0, 255), (0, 255, 0), (128, 255, 0)]
...
cv2.moveWindow(WINDOW_NAME, 200, 50)
...
while True:
    ...
    height, width = tmp_img.shape[:2]
    if zoom:
        tmp_img = zoom_resize(tmp_img=tmp_img, height=height, width=width, zoom_ratio=args.zoom_ratio)
    if unzoom:
        tmp_img = zoom_resize(tmp_img=tmp_img, height=height, width=width, zoom_ratio=1 / args.zoom_ratio)
        # tmp_img = cv2.resize(tmp_img, (int(width / args.zoom_ratio), int(height / args.zoom_ratio)))
        # height, width = tmp_img.shape[:2]

    # _, _, curr_h, curr_w = cv2.getWindowImageRect(WINDOW_NAME)
    ...
    box_coords = ((text_offset_x-6, text_offset_y-6), (text_offset_x + text_width + 2, text_offset_y + text_height + 2))
    cv2.rectangle(tmp_img, box_coords[0], box_coords[1], (255, 255, 255), cv2.FILLED)
    cv2.putText(tmp_img, text, (text_offset_x + 6, text_offset_y + 6), font, font_scale, (0, 0, 0), thickness=1)
    ...
    if pressed_key == ord('a') or pressed_key == ord('d'):
        ...
        with open(r"C:\GoogleDrive\DeepLearning\Yolo_OpenLabeling\OpenLabeling\fixable_vol3_2ndRound.txt", "a") as f:
            img_path = image_list[img_index]
            f.write(img_path + "\n")
            print("added: {} ".format(img_path))
    elif pressed_key == ord('o'):
        os.system("mspaint \"{}\"".format(img_path))
    elif pressed_key == ord('n'):
        show_only_one_per_second = not(show_only_one_per_second)
        number_of_tick = 0
    elif pressed_key == ord('x'):
        clear_all_bb()
        # Clear all bb permanently
